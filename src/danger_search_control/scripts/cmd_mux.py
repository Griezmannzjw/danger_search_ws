...
class CmdMuxCore:
    ...
    def step(
        ...
        output = (
            ...
            _limited_axis(previous[1], target[1], max_lateral_delta),
...
class CmdMux:
    """导航/电梯速度仲裁加外部急停门，且是最终 /cmd_vel 的唯一发布者。"""
    ...
    def nav_cmd_callback(self, msg):
        ...
        with self._lock:
            if values is None:
                ...
                if self._elevator_lease_active_locked(self._now_seconds()):
