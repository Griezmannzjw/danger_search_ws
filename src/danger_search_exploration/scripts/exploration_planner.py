...
class ExplorationPlanner:
    ...
    def _unknown_neighbor(self, mx, my):
        """返回前沿格 4 邻域中某个未知格 (nx, ny)；找不到返回 None。"""
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            nx, ny = mx + dx, my + dy
            if (0 <= nx < self.map_info.width and 0 <= ny < self.map_info.height
                    and self.map_data[ny, nx] == -1):
                return nx, ny
        return None
    ...
    def _select_goal(self):
        ...
        目标设在前沿的未知邻格方向：即使前沿格本身被膨胀区/未知区挡住，
        导航的 plan_toward_unknown 也会把机器人带到朝未知区最近的已知自由点，
        避免机器人卡在房间内时把所有前沿误判为不可达而提前收敛。
        """
        ...
        # 每个前沿：目标设在其未知邻格（朝未探索区域推进）；找不到未知邻格则用前沿格本身
        candidates = []
        for mx, my in representatives:
            unk = self._unknown_neighbor(mx, my)
            if unk is not None:
                candidates.append(self._map_to_world(unk[0], unk[1]))
            else:
                candidates.append(self._map_to_world(mx, my))
