...
- 初始规划和重规划期间，navigation 通过现有 20 Hz 控制定时器持续发布零速度心跳；规划短暂失败时按 `planning_failure_tolerance_s`（默认 `0.8 s`）安全重试，超时后才返回 `UNREACHABLE`。控制层 `cmd_timeout_s=0.5 s` 保持为独立的节点失联看门狗。
...
- 当目标栅格被门框或临时障碍阻挡时，Action 和 `/move_base/make_plan` 会在目标 `0.28 m` 内按 `0.05 m` 网格寻找可达安全落点。落点仍须满足原始目标的 `0.30 m` XY 容差；如果没有候选点，才报告 `UNREACHABLE`。
