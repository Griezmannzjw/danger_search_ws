...
void RecoveryAttemptTracker::acceptGoal(const std::string& goal_id)
{
  std::lock_guard<std::mutex> lock(mutex_);
  if (!has_goal_ || goal_id != goal_id_)
  {
    ++goal_epoch_;
    goal_id_ = goal_id;
    attempts_ = 0;
...
void RecoveryAttemptTracker::cancelGoal(const std::string& goal_id)
{
  std::lock_guard<std::mutex> lock(mutex_);
  if (has_goal_ && (goal_id.empty() || goal_id == goal_id_))
  {
    canceled_ = true;
  }
}
...
{
  const Pose2D unit_target = poseAt(start, maneuver, 1.0);
    ...
    double distance, double step)
{
  ...
  for (int index = 0; index <= samples; ++index)
  {
    ...
    const Pose2D pose = poseAt(start, maneuver, traveled);
