...
FAST-LIO estimates the pose of its IMU state.  In this simulator that IMU is
mounted on the pitched Livox unit rather than at the robot ``base`` frame.  A
blindly relabelled FAST-LIO pose therefore introduces a fixed translation and
orientation error.  This node obtains the fixed ``base <- livox_imu_link`` TF,
...
class LioOdometryBridge:
    def __init__(self):
        ...
        self.tracking_frame = rospy.get_param(
            "~lio_tracking_frame", "livox_imu_link"
        )
