...
class TestHectorGicpFusion(unittest.TestCase):
    ...
    def test_known_pose_mapping_keeps_identity_correction(self):
        self.fusion.update_local(1.0, 0.4, -0.2, 0.3)
        result = self.fusion.update_known_global(1.0)
        self.assertTrue(result.accepted)
        self.assertTrue(result.initialized)
        self.assertAlmostEqual(result.pose.x, 0.4)
        self.assertAlmostEqual(result.pose.y, -0.2)
        self.assertAlmostEqual(result.pose.yaw, 0.3)
        self.assertAlmostEqual(result.correction.x, 0.0)
