...
`gazebo_truth` 只能在 `competition_mode=false` 的测试中使用。
...
`map -> base` TF、结果目录可写、正式参数一致、没有禁止订阅/文件参数，以及 `/cmd_vel`
唯一发布者为 `/control`。失败时 required 节点退出，整个正式 launch 终止。
